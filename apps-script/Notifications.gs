const Notifications = {
  list:function(user,q){let a=DB.where_('Notifications',n=>String(n.userId)===String(user.id));if(q.type)a=a.filter(n=>n.type===q.type);if(String(q.unreadOnly)==='true')a=a.filter(n=>String(n.isRead)!=='true');return ok_(a.slice(-Number(q.pageSize||20)).reverse());},
